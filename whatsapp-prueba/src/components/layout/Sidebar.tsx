...
export function Sidebar({
  ...
  return (
    <aside
      className={`border-r border-slate-800 bg-slate-950 transition-all duration-300 flex flex-col h-full ${
        collapsed ? 'w-16' : 'w-64'
      }`}
    >
      {/* Collapse Button */}
      <div className="p-4 border-b border-slate-800 flex items-center justify-between">
        {!collapsed && <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Navegación</span>}
        <button
        ...
        {allowedTabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              {collapsed && (
                <div className="absolute left-full ml-2 px-2 py-1 rounded-lg bg-slate-900 border border-slate-700 text-xs font-medium text-white opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50">
                  {config.label}
                </div>
              )}
